...
# --- 2. ELIGIBILITY FILTERS & FINANCIALS ---
with st.sidebar:
    ...
    # Logic for Own Contribution percentage based on category
    ...
    with col_right:
        ...
        # Own contribution calculated automatically
        own_cont_amt = total_project_cost * own_cont_pct
        st.write(f"Own Contribution ({int(own_cont_pct*100)}%): ₹{own_cont_amt:,.0f}")
        
        # Balance to be funded
        req_term_loan = st.number_input("Term Loan Required", value=max(0.0, float(pm_cost + furn_cost + lb_cost + other_cost - own_cont_amt)))
    ...
    if total_project_cost != total_funding:
        st.error("⚠️ Project Cost and Total Funding do not match!")
...
if total_project_cost == total_funding:
    # --- VYUPY Logic ---
    if state == "Rajasthan":
        eligible_wc_loan = min(req_wc_loan, total_project_cost * 0.30)
        vyupy_loan_capped = min(req_term_loan + eligible_wc_loan, 20000000)
        v_rate = 8 if vyupy_loan_capped <= 10000000 else 7
        if is_special_cat: v_rate += 1
        vyupy_int_sub = vyupy_loan_capped * (v_rate / 100) * 5 # 5 Year Cap
    ...
    if is_new_project == "New Unit" and applicant_type == "Individual Entrepreneur" and not has_other_subsidy:
        ...
        pmegp_eligible_cost = total_project_cost - lb_cost # Land excluded
...
if results:
    ...
    # --- PROJECT FINANCING SUMMARY BOX ---
    st.markdown("---")
    st.subheader("📋 Project Financing Summary")
    c1, c2, c3 = st.columns(3)
    c1.metric("Total Project Cost", f"₹{total_project_cost:,.0f}")
    c2.metric(f"Own Contribution ({int(own_cont_pct*100)}%)", f"₹{own_cont_amt:,.0f}")
    c3.metric("Bank Loan Requirement", f"₹{(req_term_loan + req_wc_loan):,.0f}")
else:
    st.warning("Ensure Project Cost matches Total Funding to view analysis.")
